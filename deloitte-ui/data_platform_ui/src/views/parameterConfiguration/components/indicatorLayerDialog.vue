<template>
  ...
  >
    <div class="content-box">
      ...
      >
        ...
        <el-form-item label="使用场景">
          <el-checkbox-group v-model="form.businessScene" :min="0" :max="1">
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button class="btn" size="small" @click="submit" :loading="loading"
        >确 定</el-button
      >
    </span>
  </el-dialog>
...
<script>
import { addOrUpdateApply } from "@/api/paramsSeting";
export default {
  props: {
    title: {
      type: String,
  ...
  methods: {
    open() {
      ...
      // 判断有没有返回场景值 解决后端返回空值前端报错  页面显示选中还是未选中
      this.form.businessScene = this.form.businessScene == null ? [] : ["内评"];
      if (
    ...
    submit() {
      try {
        ...
        this.loading = true;
        //判断选择场景没有 //只有内评
        this.form.businessScene = this.form.businessScene.length
          ? "inside_evaluation"
          : ""; //只有内评
        addOrUpdateApply(this.form).then((res) => {
          if (res.code == 200) {
            this.$message({
              message: "操作成功",
              type: "success",
