<template>
  <div class="app-container home">
    ...
    <div class="todo-desc">
      ...
      <div>
        当前日期完成任务合计 {{ (taskCount && taskCount.taskCount) || 0 }} 条,
        待完成 {{ (taskCount && taskCount.taskNoCount) || 0 }} 条，已完成
        {{ (taskCount && taskCount.taskCop) || 0 }} 条，请尽快完成！
      </div>
    ...
    <div class="table-box">
      <table id="table" border="1" @click="getDay" />
    </div>
    ...
    <div class="list-box">
      ...
      >
        ...
        <el-table-column prop="name" label="任务状态">
          <template slot-scope="scope">
            <span
              :class="
                scope.row.complete === scope.row.taskCount ? 'green' : 'red'
              "
            >{{
              scope.row.complete !== scope.row.taskCount
                ? "待完成( " +
                  scope.row.complete +
                  "/" +
                  scope.row.taskCount +
      ...
      >
        ...
        <el-table-column prop="state" label="任务状态">
          <template slot-scope="scope">
            <span :class="scope.row.state === 0 ? 'red' : 'green'">{{
              stateArr[scope.row.state]
            }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="province" label="任务操作">
          <template slot-scope="scope">
            <el-button
              ...
              @click="addBody(scope.row)"
            >添加</el-button>
            <el-button
              :disabled="scope.row.state !== 0"
              type="text"
              size="small"
              @click="changeAddState(scope.row, 1)"
            >忽略</el-button>
            <el-button
              type="text"
              size="small"
              :disabled="scope.row.state !== 0"
              @click="deleteRole6Task(scope.row.id)"
            >删除</el-button>
            <el-button
              type="text"
              size="small"
      ...
      >
        ...
        <el-table-column prop="dataShow" label="任务状态">
          <template slot-scope="scope">
            <span :class="scope.row.state ? 'green' : 'red'">{{
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="ruleForm.cityIb === 'Y'" label="机构对应城投政府">
          ...
          <el-col class="line" :span="1">
            <el-button
              ...
              type="text"
              @click="addGovernment"
            >新增地方政府</el-button></el-col>
        </el-form-item>

        <el-form-item
          v-if="ruleForm.region"
          label="机构对应城投政府行政代码"
          prop="delivery"
        >
        ...
        <el-form-item label="敞口划分" prop="masterCode">
          ...
          >
            <el-option
              v-for="(item, index) in modelMasterList"
              :key="index"
    ...
    >
      ...
      >
        ...
        <el-form-item label="所属辖区" prop="belJurisdiction">
          <el-select
            v-model="ruleForm.belJurisdiction"
            clearable
            :filterable="true"
            class="width146"
            :multiple="xmultiple"
          ...
          >
            <el-option
              v-for="(item, index) in jurisdiction"
              :key="index"
              :label="item.value"
              :value="item.value"
            />
      ...
      <span v-if="ruleForm.state === 0" slot="footer" class="dialog-footer">
        <el-button type="primary" @click="subRule345(3)">保存并提交</el-button>
      </span>
    </el-dialog>
    <el-dialog
      title="新增地方政府"
    ...
    >
      ...
      >
        <el-form-item label="新增地方政府级别-大类">
          <el-select
            v-model="addGovForm.govLevelBig"
            :filterable="true"
            class="width320"
            @change="getSmall"
          >
        ...
        <el-form-item label="新增地方政府级别-小类">
          <el-select
            v-model="addGovForm.govLevelSmall"
            :filterable="true"
            class="width320"
            placeholder=""
          >
        ...
        <el-form-item label="上级地方政府名称" prop="delivery">
          <span
            v-if="!addGovForm.preGovName"
          >（通过下方输入官方行政编码进行查询后自动填入）</span>
          <span v-if="addGovForm.preGovName">{{ addGovForm.preGovName }}</span>
        </el-form-item>
        <el-form-item label="上级地方政府行政编码" prop="delivery">
          <el-input v-model="addGovForm.preGovCode" class="t-input width320" />
          <el-button
            style="margin-left: 5px"
            type="text"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="subGov">确认新增</el-button>
      </span>
    </el-dialog>
    <el-dialog
      title="补充城投及政府相关信息"
    ...
    >
      ...
      >
        ...
        <el-form-item label="政府持股方式" prop="shareMethod">
          <el-select
            v-model="ruleForm.shareMethod"
            :filterable="true"
            class="width320"
            placeholder="请选择"
          >
            <el-option
              v-for="(item, index) in govOptions"
              :key="index"
              :label="item.name"
              :value="item.value"
            />
        ...
        <el-form-item label="政府部门实际持股比例年份" prop="shareRatioYear">
          <el-select
            v-model="ruleForm.shareRatioYear"
            :filterable="true"
            class="width320"
            placeholder="选择年份"
          >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="subRule345(4)">保存并提交</el-button>
      </span>
    </el-dialog>
    <el-dialog
      title="补充财报收数等其他相关信息"
    ...
    <el-dialog title="IB&内评" :visible.sync="ibDig" width="50%">
      <el-form
        ref="ruleForm"
        :model="ruleForm"
        :rules="rules"
      ...
      >
        ...
        <el-form-item label="IB - SaaS - Old">
          ...
          <el-input v-if="edit1" v-model="ruleForm.name" class="t-input" />
          <el-button
            style="margin-left: 5px"
            type="text"
            @click="edit1 = !edit1"
    ...
    >
      ...
      >
        ...
        <el-form-item
          v-for="(value, key) in selectInfoRole7"
          :key="key"
          :label="key"
        >
        ...
        <el-form-item
          label="新增主体统一社会信用代码"
          :prop="!ruleForm.notUse ? 'creditCode' : ''"
        >
          <el-input
        ...
        <div class="notUse">
          ...
          <el-select
            v-model="ruleForm.creditErrorType"
            :filterable="true"
            :disabled="!ruleForm.notUse"
            placeholder="请选择"
          >
        ...
        <el-form-item>
          <el-button
            ...
            type="text"
            @click="
              check(ruleForm.entityName, ruleForm.creditCode, ruleForm.notUse)
            "
          >{{ "查重" }}
          </el-button>
          <span
            v-if="entityNamePass === 2"
            style="color: #13ce66; margin-left: 5px"
          >无重复，可新增</span>
          <a
            v-if="entityNamePass === 1"
            style="color: red; margin-left: 5px; text-decoration: underline"
            @click="showMoreData"
          >存在重复 无法新增</a>
          <el-button
            class="ml40"
          ...
          <el-button
            ...
            type="warning"
            plain
            @click="changeAddState(ruleForm, 2)"
          >忽略新增</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    ...
    >
      <div class="dig-width">
        <el-col :sm="24" :lg="12" class="form-card">
          <div class="flex1 mt10">
            ...
            <div v-if="checkStatus !== 1" class="content">
              {{ replaceData.creditCode }}
            </div>
      ...
      <div class="flex1 btn-save">
        <el-button
          ...
          type="primary"
          @click="editNew(replaceData.entityCode)"
        >修改输入为新名称
        </el-button>
        <el-button
          :disabled="checkStatus !== 2"
          type="primary"
          @click="editUsed(replaceData.entityCode, replaceData.entityName)"
        >设置输入名为曾用名
        </el-button>
      </div>
    </el-dialog>
    <el-dialog title="详情" :visible.sync="detaileDig" width="35%">
      <div class="dig-width7">
        <el-col :sm="24" :lg="12" class="form-card">
          <div
            v-for="(item, index) in selectRole7"
            :key="index"
            class="flex1 mt10"
          >
...
export default {
  ...
  methods: {
    getTypeWindList() {
      getTypeWindList({ type: 1 }).then((res) => {
        const data = res.data
        this.typeWindList = data
      })
    },
    getShenWanList() {
      getTypeWindList({ type: 2 }).then((res) => {
        const data = res.data
    ...
    init(month) {
      try {
        ...
        this.loading = true
        const mNow = month || this.monthMm - 1
        // const role2date = {
        //   date: this.nowTime,
        // };
    ...
    showMsg(flagData = []) {
      if (flagData) {
        flagData.forEach((e) => {
          const dateStr = e.taskDate.substr(-2, 2)
    ...
    work(row) {
      this.$router.push({
        ...
        query: {
          ...
          taskCateId: row.taskCateId,
          taskDate: this.clickDay || this.nowTime
        }
      })
    },
    getDay(row) {
      ...
      const clickDay =
        row.path[0].innerText >= 10
          ? row.path[0].innerText
          : '0' + row.path[0].innerText
      const tbodyObj =
        row.path[3].localName === 'tbody'
          ? row.path[3].children
          : row.path[4].children
      ...
      row.path[1].localName === 'td'
        ? (row.path[1].className = 'thisDay')
        : (row.path[2].className = 'thisDay')
      try {
        this.$modal.loading('loading...')
        if (this.roleId === 'role6') {
          const parmas = {
        ...
        getTaskCount({ TaskDate: this.monthDate + '-' + clickDay }).then(
          (res) => {
            const { data } = res
            this.taskCount = data
          }
    ...
    check(name, code, disabeld) {
      ...
      this.$modal.loading('loading...')
      try {
        validateCodeAndName({ creditCode: code, entityName: name }).then(
          (res) => {
            const { data } = res
      ...
      try {
        ...
        getTable({ id: row.id }).then((res) => {
          ...
          this.ruleForm.district =
            data.govNode && data.govNode.children.govName
          this.ruleForm.county =
            data.govNode && data.govNode.children.children.govName
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          try {
            this.$modal.loading('Loading...')
            this.ruleForm.wind =
              this.ruleForm.typeWindCheck &&
              this.ruleForm.typeWindCheck.join('--')
            this.ruleForm.shenWan =
              this.ruleForm.typeWindCheck &&
              this.ruleForm.typeShenWanCheck.join('--')
    ...
    editUsed(code, name) {
      try {
        this.$modal.loading('loading...')
        addEntityNameHis({ entityCode: code, entityNewName: name }).then(
          (res) => {
            if (res.code === 200) {
              this.$message({
                message: '操作成功',
    ...
    saveCode(code) {
      try {
        this.$modal.loading('loading...')
        editeCreditCode({ entityCode: code, creditCode: this.inputCode }).then(
          (res) => {
            if (res.code === 200) {
              this.$message({
                message: '操作成功',
