<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="${comment}" prop="taskDate">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['crm:masTask:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="masTaskList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="${comment}" align="center" prop="id" />
      <el-table-column label="德勤code" align="center" prop="entityCode" />
      <el-table-column label="${comment}" align="center" prop="taskDate" width="180">
...
<script>
import { listMasTask, getMasTask, delMasTask, addMasTask, updateMasTask } from "@/api/crm/masTask";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMasTask(this.queryParams).then(response => {
        this.masTaskList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateMasTask(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMasTask(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('crm/masTask/export', {
        ...this.queryParams
      }, `masTask_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
