    ...
    public R matchByKeyword(String keyword, String target) {
        ...
        switch (keyword) {
            ...
            case ENTITY_CODE:
                ...
                return Optional.ofNullable(entityInfo).map(r -> (R.ok(null, SuccessInfo.ENABLE_CREAT_ENTITY.getInfo()))).orElseGet(() -> R.fail(entityInfo, BadInfo.EXITS_ENTITY_CODE.getInfo()));
            //主体的统一社会信用代码
            case CREDIT_CODE:
                ...
                return Optional.ofNullable(byCreditCode).map(e -> (R.ok(null, SuccessInfo.ENABLE_CREAT_ENTITY.getInfo()))).orElseGet(() -> R.fail(byCreditCode, BadInfo.EXITS_ENTITY_CODE.getInfo()));
            //主体名称
            case ENTITY_NAME:
                EntityInfo info = entityInfoMapper.selectOne(new QueryWrapper<EntityInfo>().lambda().eq(EntityInfo::getEntityName, target));
                return Optional.ofNullable(info).map(e -> (R.ok(null, SuccessInfo.ENABLE_CREAT_ENTITY.getInfo()))).orElseGet(() -> R.fail(info, BadInfo.EXITS_ENTITY_CODE.getInfo()));
            //债券简称
            case BOND_SHORT_NAME:
                ...
                return Optional.ofNullable(bondName).map(e -> (R.ok(null, SuccessInfo.EMPTY_ENTITY_CODE.getInfo()))).orElseGet(() -> R.fail(bondName, BadInfo.EXITS_BOND_SHORT_NAME.getInfo()));
            //债券全称
            case BOND_FULL_NAME:
                ...
                return Optional.ofNullable(bondFullName).map(e -> (R.ok(null, SuccessInfo.EMPTY_ENTITY_CODE.getInfo()))).orElseGet(() -> R.fail(bondFullName, BadInfo.EXITS_BOND_SHORT_NAME.getInfo()));
            //新地方政府地方名称
            case GOV_NAME:
                GovInfo govByName = govInfoMapper.selectOne(new QueryWrapper<GovInfo>().lambda().eq(GovInfo::getGovName, target));
                return Optional.ofNullable(govByName).map(e -> (R.ok(null, SuccessInfo.EMPTY_ENTITY_CODE.getInfo()))).orElseGet(() -> R.fail(govByName, BadInfo.EXITS_GOV_NAME.getInfo()));
            //新地方政府行政编码
            case GOV_CODE:
                ...
                return Optional.ofNullable(govByCode).map(e -> (R.ok(null, SuccessInfo.EMPTY_ENTITY_CODE.getInfo()))).orElseGet(() -> R.fail(govByCode, BadInfo.EXITS_GOV_CODE.getInfo()));
            //债券代码查重
            case BOND_CODE:
                ...
                return Optional.ofNullable(bondOriCode).map(e -> (R.ok(null, SuccessInfo.EMPTY_ENTITY_CODE.getInfo()))).orElseGet(() -> R.fail(bondOriCode, BadInfo.EXITS_BOND_CODE.getInfo()));
            //A股查重
            case STOCK_CN_CODE:
                ...
                return Optional.ofNullable(stockCnInfo).map(e -> (R.ok(null, SuccessInfo.EMPTY_ENTITY_CODE.getInfo()))).orElseGet(() -> R.fail(stockCnInfo, BadInfo.EXITS_STOCK_CODE.getInfo()));
            //港股查重
            case STOCK_HK_CODE:
                ...
                return Optional.ofNullable(stockThkInfo).map(e -> (R.ok(null, SuccessInfo.EMPTY_ENTITY_CODE.getInfo()))).orElseGet(() -> R.fail(stockThkInfo, BadInfo.EXITS_STOCK_CODE.getInfo()));
            case STOCK_A_NAME:
                ...
                return Optional.ofNullable(stockCnInfoByName).map(e -> (R.ok(null, SuccessInfo.EMPTY_ENTITY_CODE.getInfo()))).orElseGet(() -> R.fail(stockCnInfoByName, BadInfo.EXITS_STOCK_SHO_NAME.getInfo()));
            case STOCK_HK_NAME:
                ...
                return Optional.ofNullable(stockThkInfoByName).map(e -> (R.ok(null, SuccessInfo.EMPTY_ENTITY_CODE.getInfo()))).orElseGet(() -> R.fail(stockThkInfoByName, BadInfo.EXITS_STOCK_SHO_NAME.getInfo()));
            default:
